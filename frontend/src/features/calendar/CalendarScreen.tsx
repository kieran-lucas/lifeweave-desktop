...
export function CalendarScreen({ selectedDate, today, onActivateDate }: Props) {
  ...
  return <PageFrame as="section" type="wide" aria-labelledby="calendar-heading">
    <PageHeader actions={<><button className={styles.actionButton} type="button" aria-label="Previous month" onClick={()=>changeMonth(-1)}>‹</button><strong aria-live="polite" className={styles.monthLabel}>{monthLabel}</strong><button className={styles.actionButton} type="button" aria-label="Next month" onClick={()=>changeMonth(1)}>›</button><button className={styles.actionButton} type="button" onClick={()=>{const date=parseDate(today);setViewMonth(date.set({day:1}));setFocusedDate(today);}}>Today</button></>}>
