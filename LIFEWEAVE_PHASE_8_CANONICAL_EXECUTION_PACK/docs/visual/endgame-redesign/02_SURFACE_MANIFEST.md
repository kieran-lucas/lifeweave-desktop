# 02 — Canonical Surface Manifest

**Canonical execution ID authority. Total: 109 rows.**

A surface/control not represented here is not authorized merely because a mockup suggests it.
For implementation scope, row ID + title + source path are authoritative. Phase 6 prose numbering is descriptive and may differ; this manifest wins for closure.
...
New rows require evidence of a materially distinct production interaction surface omitted by the trace. Discovery is not an invitation to invent capability.
