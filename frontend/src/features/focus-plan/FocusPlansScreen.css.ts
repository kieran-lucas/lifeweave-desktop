...
export const tab = style({ minHeight: "40px", padding: "8px 12px", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-control)", background: "var(--surface)", color: "inherit", cursor: "pointer", selectors: { '&[aria-selected="true"], &[aria-pressed="true"]': { outline: "2px solid var(--accent)", outlineOffset: "1px" }, "&:disabled": { opacity: 0.5, cursor: "not-allowed" } } });
...
export const listPanel = style({ border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-surface)", padding: "10px", background: "var(--surface)", maxHeight: "calc(100vh - 220px)", overflowY: "auto" });
export const planList = style({ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: "8px" });
export const planButton = style({ width: "100%", display: "grid", gap: "4px", textAlign: "left", padding: "12px", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-control)", background: "transparent", color: "inherit", cursor: "pointer", selectors: { '&[aria-current="true"]': { borderColor: "var(--accent)", background: "color-mix(in srgb, var(--accent) 8%, transparent)" } } });
export const detailPanel = style({ display: "flex", flexDirection: "column", gap: space.group, border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-surface)", padding: space.x5, background: "var(--surface)", minBlockSize: "420px", minInlineSize: 0 });
...
export const primaryButton = style({ minHeight: "40px", padding: "8px 14px", border: 0, borderRadius: "var(--radius-control)", background: "var(--accent)", color: "#fff", fontWeight: 700, cursor: "pointer", selectors: { "&:disabled": { opacity: 0.5, cursor: "not-allowed" } } });
export const secondaryButton = style({ minHeight: "36px", padding: "6px 10px", border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-control)", background: "transparent", color: "inherit", cursor: "pointer", selectors: { "&:disabled": { opacity: 0.5, cursor: "not-allowed" } } });
export const dangerButton = style([secondaryButton, { color: "var(--danger)" }]);
