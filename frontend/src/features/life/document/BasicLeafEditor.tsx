...
export default function BasicLeafEditor({ document, initialJson, onCommitted, onCancel }: Props) {
  ...
  return <section className={styles.shell} aria-label="Document editor">
    ...
    <div className={styles.actions}><button className={styles.primary} disabled={status.startsWith("Saving")} onClick={() => void commit()}>Save</button><button className={styles.button} onClick={() => { if (!dirty || window.confirm("Leave Edit? The latest recoverable draft will be kept.")) onCancel(); }}>Back to Reader</button></div>
  </section>;
}
