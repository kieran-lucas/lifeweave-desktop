...
describe("TaskWorkspaceTabs", () => {
  it("starts with Today selected and uses complete manual tab relations", async () => {
    const { container } = render(<><TaskWorkspaceTabs active="today" onActivate={vi.fn()} /><div id="task-panel-today" role="tabpanel" aria-labelledby="task-tab-today" /><div id="task-panel-upcoming" role="tabpanel" aria-labelledby="task-tab-upcoming" /><div id="task-panel-overdue" role="tabpanel" aria-labelledby="task-tab-overdue" /></>);
    const tabs = screen.getAllByRole("tab");
    expect(tabs.map((tab) => tab.textContent)).toEqual(["Today", "Upcoming", "Overdue"]);
