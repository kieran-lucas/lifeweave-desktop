...
const worldStyle = (light: NarrativeWorldPalette, dark: NarrativeWorldPalette) => style({
  ...
  background: `radial-gradient(720px 360px at 92% -6%, color-mix(in srgb, ${worldTokens.patternA} 34%, transparent), transparent 62%), radial-gradient(580px 320px at -8% 104%, color-mix(in srgb, ${worldTokens.patternB} 30%, transparent), transparent 66%), linear-gradient(135deg, color-mix(in srgb, ${worldTokens.surfaceRaised} 74%, transparent), color-mix(in srgb, ${worldTokens.canvas} 92%, transparent)), ${worldTokens.canvas}`,
  boxShadow: "var(--glow-hero)",
  "@supports": { "(backdrop-filter: blur(1px)) or (-webkit-backdrop-filter: blur(1px))": { backdropFilter: "blur(16px) saturate(1.18)", WebkitBackdropFilter: "blur(16px) saturate(1.18)" } },
  "@media": {
    "(prefers-color-scheme: dark)": { vars: vars(dark) },
    "(forced-colors: active)": { vars: { ...vars({ ...light, canvas: "Canvas", surface: "Canvas", surfaceRaised: "Canvas", text: "CanvasText", muted: "CanvasText", heading: "CanvasText", accent: "CanvasText", accentSoft: "Canvas", border: "CanvasText", rule: "CanvasText", shadow: "none", patternA: "Canvas", patternB: "Canvas", patternOpacity: "0" }) }, background: "Canvas", color: "CanvasText", boxShadow: "none", backdropFilter: "none", WebkitBackdropFilter: "none" },
  },
});
...
export const selector = style({ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 4, border: "1px solid var(--glass-border)", borderRadius: "var(--radius-control)", padding: 4, margin: "6px 0 10px", background: "var(--glass-surface)", boxShadow: "inset 0 1px 0 var(--glass-highlight)", overflow: "hidden", "@media": { "screen and (max-width: 720px)": { gridTemplateColumns: "repeat(2,minmax(0,1fr))" } } });
export const option = style({
  ...
  selectors: {
    "&:has(input:checked)": { borderColor: "color-mix(in srgb, var(--accent) 22%, var(--border-subtle))", boxShadow: "var(--glow-hover)", background: "linear-gradient(135deg, color-mix(in srgb, var(--accent-cyan) 9%, var(--active-background)), color-mix(in srgb, var(--accent-violet) 8%, var(--active-background)))" },
...
export const chip = style({
  width: 14, height: 14, borderRadius: "var(--radius-full)", border: "1px solid color-mix(in srgb, white 48%, transparent)", boxShadow: "var(--glow-dot)",
  selectors: {
