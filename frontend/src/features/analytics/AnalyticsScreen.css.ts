...
export const eyebrow = style({ margin: 0, color: "var(--accent)", fontWeight: 650, fontSize: "0.8125rem", letterSpacing: "0.065em", textTransform: "uppercase", textShadow: "0 5px 18px color-mix(in srgb, var(--accent) 18%, transparent)" });
...
globalStyle(`${primary} strong`, { ...text.numericMetric, color: "var(--accent)", textShadow: "0 10px 30px color-mix(in srgb, var(--accent) 20%, transparent)" });
globalStyle(`${primary} span`, { ...text.metadata, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em" });

export const facts = style([glass, { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 0, margin: 0, padding: space.x4, borderRadius: "var(--radius-surface)", minInlineSize: 0, borderColor: "color-mix(in srgb, var(--accent) 18%, var(--border-subtle))", boxShadow: "var(--glow-crystal)" }]);
export const summary = style([glass, { display: "flex", flexDirection: "column", margin: 0, padding: space.x5, borderRadius: "var(--radius-surface)", minInlineSize: 0, borderColor: "color-mix(in srgb, var(--accent-violet) 22%, var(--border-subtle))", background: "radial-gradient(520px 220px at 96% 4%, color-mix(in srgb, var(--accent-violet) 13%, transparent), transparent 68%), radial-gradient(420px 180px at 8% 100%, color-mix(in srgb, var(--accent-cyan) 11%, transparent), transparent 68%), var(--glass-surface)", boxShadow: "var(--glow-hero)" }]);
export const summaryFacts = style({ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 0, margin: 0, minInlineSize: 0, paddingBlockStart: space.x3, marginBlockStart: space.x4, borderBlockStart: "1px solid color-mix(in srgb, var(--accent) 14%, var(--border-subtle))" });
...
globalStyle(`${secondaryGrid} > section:first-child > ul`, { listStyle: "none", display: "grid", gap: space.x2, margin: 0, padding: space.x4, border: "1px solid var(--glass-border)", borderRadius: "var(--radius-surface)", background: "var(--glass-surface-strong)", boxShadow: "var(--glow-crystal)", minInlineSize: 0 });
...
export const categories = style([glass, { listStyle: "none", padding: space.x4, margin: 0, display: "grid", gap: space.field, borderRadius: "var(--radius-surface)", minInlineSize: 0, boxShadow: "var(--glow-crystal)" }]);
...
export const module_ = style([glass, { padding: space.x4, borderRadius: "var(--radius-surface)", minInlineSize: 0, boxShadow: "var(--glow-crystal)" }]);
