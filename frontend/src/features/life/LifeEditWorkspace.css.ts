...
export const canvasViewport=style({position:"relative",minWidth:0,minHeight:520,overflow:"auto",border:"1px solid var(--glass-border)",borderRadius:"var(--radius-floating)",background:"color-mix(in srgb, var(--surface) 75%, var(--app-background))",scrollbarGutter:"stable"});
...
export const nodeCard=style({display:"grid",gridTemplateColumns:"30px minmax(0,1fr)",alignItems:"center",gap:8,width:"100%",minHeight:66,padding:"9px 10px",border:"1px solid var(--glass-border)",borderRadius:"var(--radius-surface)",background:"var(--glass-surface-strong)",color:"var(--text-primary)",textAlign:"left",cursor:"grab",selectors:{"&[aria-pressed=true]":{borderColor:"var(--focus-ring)",boxShadow:"0 0 0 2px color-mix(in srgb, var(--focus-ring) 25%, transparent)"},"&:focus-visible":{outline:"3px solid var(--focus-ring)",outlineOffset:2},"&:active":{cursor:"grabbing"}}});
...
export const inspector=style({position:"sticky",top:0,display:"grid",gap:12,padding:16,border:"1px solid var(--glass-border)",borderRadius:"var(--radius-surface)",background:"var(--glass-surface-strong)"});
