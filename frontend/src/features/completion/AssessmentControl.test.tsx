...
describe("equal-emphasis assessment fan",()=>{
 ...
 it("renders the four states in stable order with equal option classes",()=>{renderControl();fireEvent.click(screen.getByRole("button",{name:/Assess task/}));const options=screen.getAllByRole("option");expect(options.map(option=>option.textContent)).toEqual(states.map(state=>state.label));expect(new Set(options.map(option=>option.className)).size).toBe(1);});
