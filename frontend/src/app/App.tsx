    ...
    <div className={styles.appRoot} data-sidebar-mode={collapsed ? "collapsed" : "expanded"}>
      <nav className={styles.sidebar} aria-label="Primary navigation">
        ...
        <div className={styles.navGroup}>
          ...
          <div className={styles.divider} />
          <button ref={searchTriggerRef} type="button" className={styles.navButton} onClick={() => setSearchOpen(true)} aria-label="Search (Ctrl+K)">
            <span aria-hidden="true" className={styles.navIcon}>⌕</span><span className={styles.navLabel}>Search</span>
          </button>
        </div>
      ...
      <main className={styles.viewport}>
        ...
        {ipcStatus === "ready" && <RouteErrorBoundary key={destination} destination={destination}>
          ...
          {destination === "today" && <div ref={(node) => { headingRef.current = node; }}><TodayScreen selectedDate={selectedDate} onSelectedDateChange={setSelectedDate} focusRequest={pendingNav?.target.kind === "today" ? { requestId: pendingNav.requestId, taskId: pendingNav.target.task_id, seriesId: pendingNav.target.series_id } : null} /></div>}
          ...
          {destination === "life" && <div ref={(node) => { headingRef.current = node; }}><LifeScreen entryRequest={pendingNav?.target.kind === "life_browse" || pendingNav?.target.kind === "life_reader" ? { requestId: pendingNav.requestId, nodeId: pendingNav.target.node_id, mode: pendingNav.target.kind === "life_reader" ? "reader" : "browse" } : null} /></div>}
        </RouteErrorBoundary>}
      </main>
      {searchOpen && (
        <Suspense>
