...
/** Structural edit canvas is a matte drafting board continuous with Life Browse. */
...
export const nodeCard = style([focusRing, {
  ...
  cursor: "grab",
  selectors: {
    "&[aria-pressed=true]": { borderInlineStart: "3px solid var(--accent)", borderColor: "var(--paint-edge-strong)", backgroundColor: "var(--paint-selected)", backgroundImage: "var(--paint-grain-fine), var(--paint-wash-blue)", boxShadow: "var(--glow-selected)" },
...
export const input = style([focusRing, { width: "100%", minWidth: 0, minBlockSize: 36, border: "1px solid var(--paint-edge)", borderRadius: "var(--radius-control)", padding: "7px 10px", backgroundColor: "var(--paint-sheet)", backgroundImage: "var(--paint-grain-fine)", color: "var(--text-primary)", font: "inherit" }]);
...
export const overlay = style({ width: 164, padding: 10, border: "2px solid var(--focus-ring)", borderRadius: "var(--radius-surface)", backgroundColor: "var(--paint-sheet-strong)", backgroundImage: "var(--paint-grain-fine), var(--paint-wash-blue)", boxShadow: "var(--elevation-modal)", color: "var(--text-primary)", fontWeight: 750 });
export const preview = style({ stroke: "var(--focus-ring)", strokeWidth: 2, strokeDasharray: "5 4", fill: "none" });
globalStyle(`${links} path`, { stroke: "color-mix(in srgb, var(--accent) 22%, var(--paint-edge-strong))", strokeWidth: 1.25, fill: "none" });
