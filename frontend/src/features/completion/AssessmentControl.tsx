...
export function AssessmentControl({itemId,states,evaluation,eligible,open,onOpen,onClose,onSelect}:Props){
  ...
  const available=eligible&&states.length>0;
  ...
  return <div className={styles.anchor}>
    <button ref={trigger} type="button" className={styles.trigger} data-state={evaluation?.visual_token??"empty"} aria-label={!eligible?`Assessment unavailable until task ends`:states.length===0?"Assessment options are loading":`Assess task. Current state: ${label}`} aria-haspopup="listbox" aria-expanded={open} disabled={!available} onClick={event=>{event.stopPropagation();open?close():onOpen();}}><span className={styles.ring} aria-hidden="true"/></button>
    {open&&createPortal(<div ref={fan} className={styles.fan} role="listbox" aria-label="Completion assessment" data-item={itemId}>{states.map((state,index)=><button key={state.id} type="button" role="option" aria-selected={evaluation?.state_id===state.id} aria-label={state.label} data-option={index} data-visual={state.visual_token} tabIndex={index===active?0:-1} className={styles.option} onKeyDown={event=>key(event,index)} onClick={()=>choose(state)}>{state.label}</button>)}</div>,document.body)}
  </div>;
}
