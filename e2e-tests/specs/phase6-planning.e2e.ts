...
describe("Phase 6 — Upcoming and Overdue planning", () => {
  it("projects bounded queues, opens exact recurring identity, and reviews an overdue task", async () => {
    ...
    const fixture = await browser.execute(async () => {
      ...
      const cancelledSeries = await invoke<string>("create_recurring_task", { input: { title: "E2E Cancelled Past", description: "cancelled", local_date: yesterday, start_minute: 720, end_minute: 780, category_id: "general", priority: "low", frequency: "daily", interval: 1, weekdays: [], until: null, count: 1, life_node_id: null } });
      ...
      const movedSeries = await invoke<string>("create_recurring_task", { input: { title: "E2E Moved Future", description: "moved", local_date: tomorrow, start_minute: 840, end_minute: 900, category_id: "general", priority: "low", frequency: "daily", interval: 1, weekdays: [], until: null, count: 1, life_node_id: null } });
