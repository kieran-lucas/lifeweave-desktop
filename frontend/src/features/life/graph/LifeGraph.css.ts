...
export const workspace = style([splitWorkspace, { vars: { "--lw-split-columns": "minmax(0,1fr) minmax(280px,320px)" }, gap: space.x4 }]);
export const header = style({ gridColumn: "1/-1", display: "flex", gap: 12, alignItems: "baseline", flexWrap: "wrap" });
...
export const nodeTitle = style({ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", ...text.cardTitle });
...
export const inspectorMeta = style({ margin: 0, display: "grid", gap: 4, color: "var(--text-muted)", ...text.metadata });
export const connectionGroup = style({ display: "grid", gap: 6 });
export const connectionHeading = style({ margin: 0, ...text.cardTitle });
export const connectionList = style({ display: "grid", gap: 5, listStyle: "none", padding: 0, margin: 0, maxHeight: 190, overflow: "auto" });
export const connectionButton = style([focusRing, {
  width: "100%",
  border: "1px solid transparent",
  borderLeft: "3px solid var(--paint-edge)",
