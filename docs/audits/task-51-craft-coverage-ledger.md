...
- `VR` — 38 tracked goldens under `e2e-tests/visual-baselines/windows-webview2/` plus zero-diff
  comparison evidence `task51-visual-baseline-verify2-1536-20260809` and
  `task51-visual-baseline-verify-960-20260809` under `target/e2e-artifacts/task-50b/`
...
|---|---|---|---|---|---|---|
| App shell — startup or any destination | sidebar expanded/collapsed, active/hover/focus nav, separators, atmosphere, scrollbars | app frame, page frame, icon buttons, atmosphere | Y/N/Y/P/N | RMAX `01-today.png`, `24-today-collapsed.png`; R960 equivalents; GEO 28×2 | PARTIAL | Dark, forced colors, scrollbar and complete keyboard walk remain. |
...
| Global theme/motion/language axes | light, dark, reduced motion, Vietnamese, long text, DPI | theme contract, motion contract, typography roles | P/N/P/P/N | current light screenshots; existing DOM contracts | PARTIAL | Dark, forced colors, reduced-motion render, Vietnamese surface proof, Narrator and physical DPI remain. |

## Task workspace

| Surface / entry | Primary states and nested UI | Shared primitives | L/D/N/K/F | Evidence | Status | Defects / next proof |
|---|---|---|---|---|---|---|
| Today — sidebar `Today`, workspace tab `Today` | populated, dense/long titles, completed, selected/unselected, no selection, empty/loading/error, timer-running | page header, week strip, row group, chips, buttons, assessment control | Y/N/Y/P/N | RMAX/R960 `01-today.png`; GEO 28×2; DOM | PARTIAL | Current fixture proves dense rows but not empty/loading/error/timer/completed or dark/forced-colors. |
...
| Task create/edit dialog — `Plan task` or row `Edit` | empty, populated, recurrence, occurrence scope, validation, deletion, date/time, comboboxes, tags | dialog grammar, fields, select, checkbox, combobox, chips | Y/N/Y/P/N | RMAX/R960 `02`–`04`; DOM | PARTIAL | Validation/delete/recurrence-scope states, dark/forced colors and keyboard focus containment need current captures. |
...
| Surface / entry | Primary states and nested UI | Shared primitives | L/D/N/K/F | Evidence | Status | Defects / next proof |
|---|---|---|---|---|---|---|
| Calendar — sidebar `Calendar` | ordinary/six-week month, current/selected/outside day, dense/empty days, missed state, load bars, month controls | page header, grid buttons, progress material | Y/N/Y/Y/N | RMAX/R960 `09-calendar.png`; DOM 13 Calendar tests; GEO | VERIFIED | Recovery `efcb1a2`: 96px cells and 5px load bars. Global dark/FC axes remain tracked above. |
| Analytics — sidebar `Analytics` | week/month/year, period navigation, summary, planned-vs-actual, categories, distribution, streaks, Focus Plan activity, empty/loading | compact period group, tabs, progress, tables, loading/empty states | Y/N/Y/P/N | RMAX/R960 `10-analytics.png`; focused DOM; GEO | PARTIAL | Recovery fixed stretched toolbar. Still capture month/year, empty, plan table local scroll, dark/FC and keyboard. |
...
| Surface / entry | Primary states and nested UI | Shared primitives | L/D/N/K/F | Evidence | Status | Defects / next proof |
|---|---|---|---|---|---|---|
| Life Browse — sidebar `Life System`, `Browse` | focal node, direct children, leaf/no children, pinned state, long descriptions | mode tabs, breadcrumbs, node/child actions | Y/N/Y/P/N | LIFEP3 `12-life-browse.png`; R960; GEO | PARTIAL | Focal and children now form one bounded, left-anchored hierarchy with the shared icon vocabulary. Leaf/no-child, pagination, long/dense and dark/FC states remain. |
...
| Life Graph — `Graph` from Browse | nodes/edges/labels, selection/hover/focus, details, dense graph, relationship table | local-scroll canvas, SVG graph, select, details list | Y/N/Y/P/N | LIFEP3 `15-life-graph.png`; R960; GEO; DOM | PARTIAL | Selection is now tonal and the inspector/relationship rows use the shared low-chrome grammar. Add explicit-link/dense/error states and dark/FC. Zoom/pan is `N/A`: no such production behavior exists and Task 51 cannot add Graph semantics. Empty is `N/A`: the projection always contains the Life root. |
...
| Basic Leaf Editor — Reader `Edit document` | empty/long document, headings/lists/links/code/quotes, selection/caret, toolbar, outline, save/error | Tiptap, editor toolbar, reading page | Y/N/N/Y/N | RDL `17-basic-editor.png`, `17b-basic-editor-link-dialog.png`; DOM 62-test modal/Reader/Life subset; GEO | PARTIAL | Real no-save entry/exit and shared link dialog now render with Escape/focus-return proof; toolbar/control grammar was quieted. Authored content, selection/caret, dirty-exit visual, save/error and dark/narrow/FC remain. |
...
|---|---|---|---|
| Deterministic visual regression | `@wdio/visual-service` now compares 38 tracked, fixture-backed production goldens across canonical `1536×794` and achieved-minimum `959×639`: all major route families, representative task/editor/Narrative decisions, Search results/no-results and Keyboard Help. Acceptance is off by default, runtime diffs are isolated under `target`, environment metadata is tracked, both VR passes compare at exactly 0% mismatch, and a timestamp-backed nondeterminism defect was found and corrected rather than masked. | PARTIAL | Add representative dark baselines plus remaining empty/dense and edge-flow coverage; retain zero mismatch and the explicit acceptance policy. |
