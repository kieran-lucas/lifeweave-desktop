 ...
 * The period name uses the card-title scale so it reads as a real timeline section, while the
 * range stays quiet metadata and cannot compete with the task titles below it.
 */
export const periodHeading=style({display:"flex",flexWrap:"wrap",alignItems:"center",gap:space.control,margin:0,minInlineSize:0});
export const periodLabel=style({display:"inline-flex",alignItems:"center",gap:space.x1,minInlineSize:0,...text.cardTitle,textTransform:"uppercase",letterSpacing:"0.035em"});
export const periodIcon=style({inlineSize:19,blockSize:19,flexShrink:0,color:"var(--accent)"});
