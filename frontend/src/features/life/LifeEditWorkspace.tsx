...
export function LifeEditWorkspace({initialNodeId,onBrowse}:{initialNodeId:string;onBrowse:(id:string)=>void}){
 ...
 useEffect(()=>{if(initialPositioned.current)return;const viewport=viewportRef.current;const point=layout.points.get(initialNodeId);if(!viewport||!point)return;viewport.scrollLeft=Math.max(0,point.x-30);viewport.scrollTop=Math.max(0,point.y-62);initialPositioned.current=true;},[initialNodeId,layout]);
 ...
 return <>
  <span id="life-edit-instructions" className={styles.srOnly}>Node drag and drop is disabled. Open Edit node to use the explicit Move controls.</span><span id="life-pan-instructions" className={styles.srOnly}>Hold and drag empty tree space to move the canvas. Arrow keys pan it, Shift plus an arrow pans farther, and Home returns to the origin.</span>
