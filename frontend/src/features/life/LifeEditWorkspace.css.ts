...
export const nodeCard=style({display:"grid",gridTemplateColumns:"30px minmax(0,1fr)",alignItems:"center",gap:8,width:"100%",minHeight:66,padding:"9px 10px",border:"1px solid var(--border-subtle)",borderRadius:12,background:"var(--surface)",color:"var(--text-primary)",textAlign:"left",cursor:"grab",selectors:{"&[aria-pressed=true]":{borderColor:"var(--focus-ring)",boxShadow:"0 0 0 2px color-mix(in srgb, var(--focus-ring) 25%, transparent)"},"&:focus-visible":{outline:"3px solid var(--focus-ring)",outlineOffset:2},"&:active":{cursor:"grabbing"}}});
...
export const inspector=style({position:"sticky",top:0,display:"grid",gap:12,padding:16,border:"1px solid var(--border-subtle)",borderRadius:16,background:"var(--surface)"});
...
export const input=style({width:"100%",minWidth:0,border:"1px solid var(--border-subtle)",borderRadius:9,padding:"8px 10px",background:"var(--app-background)",color:"var(--text-primary)",font:"inherit",selectors:{"&:focus-visible":{outline:"3px solid var(--focus-ring)",outlineOffset:1}}});
export const actions=style({display:"flex",gap:7,flexWrap:"wrap"});
export const button=style({border:"1px solid var(--border-subtle)",borderRadius:9,padding:"7px 10px",background:"var(--app-background)",color:"var(--text-primary)",cursor:"pointer",fontWeight:650,selectors:{"&:disabled":{opacity:.45,cursor:"default"},"&:focus-visible":{outline:"3px solid var(--focus-ring)",outlineOffset:2}}});
...
export const overlay=style({width:164,padding:10,border:"2px solid var(--focus-ring)",borderRadius:12,background:"var(--surface)",boxShadow:"0 12px 30px color-mix(in srgb, var(--text-primary) 16%, transparent)",color:"var(--text-primary)",fontWeight:750});
