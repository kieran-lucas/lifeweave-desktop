...
| Static visual residue | Current ratchet passes while permitting 12 feature hex values, 14 literal feature transitions, 27 candidate glyph-icon lines, prototype-only shared button recipes, duplicate tabs/comboboxes, and native select/search affordance questions. | PARTIAL | Migrate or explicitly justify every hit, add static guards where appropriate, and render the production control matrix on WebView2. |
