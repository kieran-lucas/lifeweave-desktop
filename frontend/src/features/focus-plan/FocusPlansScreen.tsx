          ...
          {!selected || !form ? (
            ...
            <>
              ...
              <section aria-labelledby="variants-heading">
                ...
                <div className={styles.variantTabs}>{selected.variants.map((variant) => <button key={variant.id} type="button" className={styles.tab} aria-pressed={variant.id === selected.selected_variant_id} disabled={variant.archived || status === "saving"} onClick={() => void runMutation({ action: "select_variant", variant_id: variant.id })}>{variant.label}{variant.archived ? " (archived)" : ""}</button>)}</div>
                ...
                {selectedVariant && <div className={styles.variantEditor}>
                  <div className={styles.inlineForm}><input className={styles.input} value={variantLabel} onChange={(event) => setVariantLabel(event.target.value)} /><button type="button" className={styles.secondaryButton} disabled={!variantLabel.trim()} onClick={() => void runMutation({ action: "rename_variant", variant_id: selectedVariant.id, label: variantLabel.trim() })}>Rename</button>{selected.variants.length > 1 && <button type="button" className={styles.dangerButton} onClick={() => void runMutation({ action: selectedVariant.archived ? "restore_variant" : "archive_variant", variant_id: selectedVariant.id })}>{selectedVariant.archived ? "Restore" : "Archive"}</button>}</div>
