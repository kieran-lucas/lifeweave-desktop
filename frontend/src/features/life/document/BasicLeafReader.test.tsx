...
describe("Basic Leaf Reader",()=>{
 ...
 it("shows Create Basic Leaf document and Create Narrative Canvas for an empty leaf",async()=>{api.get.mockResolvedValue(projection({document:null}));mount();expect(await screen.findByRole("button",{name:"Create Basic Leaf document"})).toBeInTheDocument();expect(await screen.findByRole("button",{name:"Create Narrative Canvas"})).toBeInTheDocument();});
