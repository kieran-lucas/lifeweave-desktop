...
|---|---|---|---|
| Deterministic visual regression | `@wdio/visual-service` compares 97 tracked, fixture-backed production goldens across canonical light `1536×794`, achieved-minimum light `959×639`, canonical dark `1536×794`, canonical forced colors and Vietnamese typography. The E2E presentation date is pinned without altering the native concurrency date, comparison frames are retained, and focused text controls are blurred/restored only while pixels are compared to remove WebView2's nondeterministic 27-pixel caret. Coverage includes all major route families, timer/assessment, TagPicker, Saved View editor, Life Link dialog, Life interchange previews, the empty Reader, all Studio worlds, editor decisions, Search and Keyboard Help. Acceptance is off by default, runtime diffs stay under `target`, tag-scoped runs retain exact-zero comparison without weakening thresholds, and timestamped restore evidence is intentionally not golden. | PARTIAL | Add remaining empty/dense/error and narrow/FC edge-flow coverage; retain zero mismatch and the explicit acceptance policy. |
