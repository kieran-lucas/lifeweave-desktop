...
| Surface / entry | Primary states and nested UI | Shared primitives | L/D/N/K/F | Evidence | Status | Defects / next proof |
|---|---|---|---|---|---|---|
| Life Browse — sidebar `Life System`, `Browse` | focal node, direct children, leaf/no children, pinned state, long descriptions | mode tabs, breadcrumbs, node/child actions | Y/N/Y/P/N | RMAX/R960 `12-life-browse.png`; GEO | PARTIAL | Sparse composition remains card-heavy with oversized dead space and mismatched icon marks. |
...
| Release geometry assertion | `task50b-maximized-audit.e2e.ts` records achieved inner viewports and is credible evidence, but `phase21-global-layout.e2e.ts` still expects Today to be `standard` rather than its approved `wide` taxonomy and requests outer sizes without proving achieved inner sizes. | PARTIAL | Update the stale release phase to current authority, reuse achieved-inner-viewport verification, and assert collision/overflow totals are zero rather than only reporting them. |
