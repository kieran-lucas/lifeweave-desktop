...
- `VR` — 105 tracked light/maximized, light/minimum, dark/maximized, forced-colors and Vietnamese goldens under
...
| Basic Leaf Reader — open documented leaf | reading measure, outline, links/related tasks, long/Vietnamese/empty/error | reading page, Literata roles, outline, link panels | Y/Y/Y/P/Y | RPR; VR `16-life-reader` light/dark/FC/Vietnamese and `16c-life-empty` light/dark; DOM; GEO | PARTIAL | Deterministic root → Layout Area → documented-leaf and empty-leaf paths work. Vietnamese title, outline, uppercase hierarchy, bold/italic body, list and the empty chooser are verified; error and a longer multi-page document remain. |
...
| Life package flows — Life controls | portable, branch, whole-tree export/import previews, warnings/errors/confirmations | modal grammar, file controls, tables/lists | Y/Y/N/P/N | VR `13b`, `13c`, `16e` light/dark; 51-state native light/dark walks; DOM; GEO | PARTIAL | Production export bytes are reinjected into the real import controls and every preview is cancelled without mutation. Tree-preview spacing was corrected so warnings and actions fit the canonical viewport. Add narrow/FC, blocked/error and confirm-path evidence. |
...
| Narrative markdown import/export | import dialog, preview/warnings/errors, export | modal grammar, file controls, buttons | Y/Y/N/P/N | VR `16d-markdown-import` light/dark; 51-state native light/dark walks; DOM; GEO | PARTIAL | A real in-memory Markdown file reaches the production preview and is cancelled without mutation. Add narrow/FC, invalid/warning and confirm-path evidence. |
...
| Restore confirmation — Settings `Restore` | destructive confirmation, compatibility warning, failure | shared dialog, danger button, status | Y/Y/N/P/N | runtime `23b-restore-confirmation` light/dark; DOM; GEO | PARTIAL | The newest real managed-backup row opens the production confirmation and cancels safely; timestamped screenshots remain runtime evidence rather than unstable goldens. Add narrow/FC and failure/pending evidence. |
...
| Non-modal popups | assessment fan; Life-area/Focus Plan comboboxes; TagPicker disclosure region | listbox/combobox/radial portal | Y/Y/Y/Y/Y | VR `01e`, `02b`, `02c`, `02d` light/dark/FC/minimum; DOM; static census | PARTIAL | Assessment, TagPicker and both Task combobox open states are inspected; loading/error/no-match/selection edge captures remain. |
...
| Task nested comboboxes | Life-area and Focus Plan closed/open/loading/error/no-match/selected | shared implementation; VR `02c`/`02d` light/dark/FC/minimum; DOM Escape/blur/selection coverage | PARTIAL |
...
| Life interchange controls | branch/tree/portable export blocked/success/error and import preview/warning/error | branch/tree/portable production preview light/dark plus DOM | PARTIAL |
| Basic Reader/Editor | empty-leaf chooser; dual-document error; draft available/conflict; outline collapsed/expanded/truncated; asset loading/failure; dirty exit/link prompt | empty chooser and link/dirty-decision light/dark plus DOM | PARTIAL |
...
| Settings backup/Foundation | create pending/cleanup notice/list error/compatibility variants/restore pending/error; Foundation loading/error/empty/create/edit/archive/restore | backup list plus real restore-confirmation light/dark captures and functional tests | PARTIAL |
...
| Task deletion | matrix row 1.24 requires confirmation; runtime currently deletes directly from the edit dialog | A shared-dialog implementation was behavior-tested, then removed after measured production build evidence exceeded the locked `total_js_bytes` maximum by 688 bytes at its smallest implementation. Repository policy requires a Product Owner ceiling decision before this semantic surface can ship. | NOT REVIEWED |
...
|---|---|---|---|
| Deterministic visual regression | `@wdio/visual-service` compares 105 tracked, fixture-backed production goldens across canonical light `1536×794`, achieved-minimum light `959×639`, canonical dark `1536×794`, canonical forced colors and Vietnamese typography. The E2E presentation date is pinned without altering the native concurrency date, comparison frames are retained, and a focused text-editable caret is temporarily made transparent while pixels are compared. Coverage includes all major route families, timer/assessment, TagPicker, both Task comboboxes, Saved View editor, Life Link dialog, Life interchange previews, the empty Reader, all Studio worlds, editor decisions, Search and Keyboard Help. Acceptance is off by default, runtime diffs stay under `target`, tag-scoped runs retain exact-zero comparison without weakening thresholds, and timestamped restore evidence is intentionally not golden. | PARTIAL | Add remaining empty/dense/error and narrow/FC edge-flow coverage; retain zero mismatch and the explicit acceptance policy. |
...
| Production dark / forced-colors / reduced-motion | The DevTools-scoped WebView2 media override independently hard-asserts production `matchMedia` for all three axes. Dark supplies inspected zero-diff goldens across the major surfaces; forced colors supplies 12 inspected zero-diff goldens covering structure, native controls, both Task comboboxes, graph, editor/dialog, Settings, Search and Keyboard Help. Reduced motion passed the full 36-state interaction/geometry walk; its five representative static captures matched the light goldens byte-for-byte and therefore are retained as labeled runtime evidence rather than duplicate tracked PNGs. | VERIFIED | Keep all media preconditions and exact-zero comparisons green. |
...
At the current local import/edge checkpoint, the real-app evidence is:

```text
light 1536×794       51 states   0 collisions   0 document overflow   0 viewport overflow
dark  1536×794       51 states   0 collisions   0 document overflow   0 viewport overflow
tracked visual set   105 exact-zero goldens across all established axes
