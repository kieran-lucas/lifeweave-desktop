...
export function NarrativeTemplateChooser({ nodeId }: { nodeId: string }) {
  ...
  const create = useMutation({ mutationFn: () => { attempt.current ??= operationId("narrative-create"); return createNarrativeDocument({ life_node_id: nodeId, operation_id: attempt.current, template_id: selected }); }, onSuccess: () => void client.invalidateQueries({ queryKey: narrativeKey(nodeId) }) });
  if (!open) return <button ref={trigger} className={styles.primary} onClick={() => setOpen(true)}>Create Narrative Canvas</button>;
