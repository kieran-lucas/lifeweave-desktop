...
- `VR` — 63 tracked light/maximized, light/minimum, dark/maximized and forced-colors goldens under
...
| Global theme/motion/language axes | light, dark, reduced motion, Vietnamese, long text, DPI | theme contract, motion contract, typography roles | Y/Y/Y/P/Y | VR light/dark/minimum/FC; 36-state reduced-motion native walk; existing DOM contracts | PARTIAL | Vietnamese surface proof, Narrator and physical DPI remain. |

## Task workspace

| Surface / entry | Primary states and nested UI | Shared primitives | L/D/N/K/F | Evidence | Status | Defects / next proof |
|---|---|---|---|---|---|---|
| Today — sidebar `Today`, workspace tab `Today` | populated, dense/long titles, completed, selected/unselected, no selection, empty/loading/error, timer-running | page header, week strip, row group, chips, buttons, assessment control | Y/Y/Y/P/Y | RMAX/R960; VR `01-today` light/dark/FC; GEO; DOM | PARTIAL | Current fixture proves dense rows but not empty/loading/error/timer/completed. |
...
| Editor decision dialogs — Reader/Studio destructive/link actions | Basic link prompt, dirty exits, only-block alert, block/scene delete confirmations | shared decision dialog and modal focus trap | Y/P/P/Y/P | RDL/VR `17b-basic-editor-link-dialog` light/dark/FC/narrow; RNA `19b-narrative-only-block-dialog.png`; DOM | PARTIAL | No production browser prompt/alert/confirm remains. Basic Link and Narrative protection dialogs render in the real app; DOM proves dirty exits and destructive cancel/confirm paths. Real dirty/destructive captures across all axes remain. |
...
|---|---|---|---|
| Deterministic visual regression | `@wdio/visual-service` now compares 63 tracked, fixture-backed production goldens across canonical light `1536×794`, achieved-minimum light `959×639`, canonical dark `1536×794`, and canonical forced colors: all major route families, representative task/editor/Narrative decisions, Search and Keyboard Help. Acceptance is off by default, runtime diffs are isolated under `target`, environment metadata is tracked, all four VR passes compare at exactly 0% mismatch, and a timestamp-backed nondeterminism defect was corrected rather than masked. | PARTIAL | Add remaining empty/dense and edge-flow coverage; retain zero mismatch and the explicit acceptance policy. |
