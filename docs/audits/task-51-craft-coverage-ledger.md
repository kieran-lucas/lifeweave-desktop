...
| Browser-native editor dialogs — Reader/Studio destructive/link actions | Basic link prompt, dirty exits, only-block alert, block/scene delete confirmations | currently `window.prompt` / `alert` / `confirm` | N/N/N/N/N | static census | NOT REVIEWED | HIGH legacy island: replace with the shared dialog grammar without changing persistence semantics; prove Tab/Escape/restore, dark/FC/narrow. |
