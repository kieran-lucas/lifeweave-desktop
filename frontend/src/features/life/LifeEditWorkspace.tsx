...
function Positioner({node,point,selected,menuOpen,onSelect,onAdd,onEdit,invalid,active}:{node:LifeEditNodeView;point:Point;selected:boolean;menuOpen:boolean;onSelect:()=>void;onAdd:()=>void;onEdit:()=>void;invalid:boolean;active:boolean}){
...
export function LifeEditWorkspace({initialNodeId,onBrowse}:{initialNodeId:string;onBrowse:(id:string)=>void}){
 ...
 const [selectedId,setSelectedId]=useState(initialNodeId);const [menuNodeId,setMenuNodeId]=useState<string>();const [editorOpen,setEditorOpen]=useState(false);const [createOpen,setCreateOpen]=useState(false);const [activeId,setActiveId]=useState<string>();const [overId,setOverId]=useState<string>();const [message,setMessage]=useState("");
 ...
 const canvasRef=useRef<HTMLDivElement|null>(null);const viewportRef=useRef<HTMLDivElement|null>(null);const panSession=useRef<{pointerId:number;x:number;y:number;left:number;top:number}|null>(null);const focusAfter=useRef<string|undefined>(undefined);const menuAfter=useRef<string|undefined>(undefined);const dialogReturnFocus=useRef<HTMLElement|null>(null);
 ...
 useEffect(()=>{if(!menuNodeId)return;const closeOnOutside=(event:PointerEvent)=>{const target=event.target;if(target instanceof Element&&(target.closest("[data-life-node-actions]")||target.closest("[data-life-edit-id]")))return;setMenuNodeId(undefined);};const closeOnEscape=(event:KeyboardEvent)=>{if(event.key!=="Escape")return;event.preventDefault();const id=menuNodeId;setMenuNodeId(undefined);requestAnimationFrame(()=>document.querySelector<HTMLElement>(`[data-life-edit-id="${id}"]`)?.focus({preventScroll:true}));};document.addEventListener("pointerdown",closeOnOutside);document.addEventListener("keydown",closeOnEscape);return()=>{document.removeEventListener("pointerdown",closeOnOutside);document.removeEventListener("keydown",closeOnEscape);};},[menuNodeId]);
 ...
 return <DndContext sensors={sensors} collisionDetection={closestCenter} accessibility={{screenReaderInstructions:{draggable:"To move a Life node, press Space. Use arrow keys to choose a valid target, Space to commit, or Escape to cancel."}}} onDragStart={event=>{setMenuNodeId(undefined);setActiveId(event.active.id.toString().replace("node:",""));setMessage("Moving Life node. Descendants and the node itself are unavailable targets.");}} onDragOver={onDragOver} onDragCancel={()=>{setActiveId(undefined);setOverId(undefined);setMessage("Move cancelled.");}} onDragEnd={onDragEnd}>
  <HiddenText id="life-edit-instructions" value="Use drag and drop or open Edit node for Move controls. Rust validates every parent and prevents cycles."/><HiddenText id="life-pan-instructions" value="Hold and drag empty tree space to move the canvas. Arrow keys pan it, Shift plus an arrow pans farther, and Home returns to the origin."/><LiveRegion id="life-edit-live" announcement={message} ariaLiveType="polite"/>
  <div className={styles.workspace} data-editor-open={editorOpen?"true":"false"}>
   <div ref={viewportRef} className={styles.canvasViewport} aria-label="Full Life tree editor" aria-describedby="life-pan-instructions" aria-keyshortcuts="ArrowUp ArrowDown ArrowLeft ArrowRight Home" tabIndex={0} data-life-pan-surface="" data-panning="false" data-reduced-motion={reduced?"true":"false"} onPointerDown={startPan} onPointerMove={movePan} onPointerUp={stopPan} onPointerCancel={stopPan} onLostPointerCapture={stopPan} onKeyDown={panWithKeyboard}>
    <div ref={canvasRef} className={styles.canvas} role="tree" aria-label="Full Life tree editor" aria-describedby="life-edit-instructions">
     <svg className={styles.links} width={layout.width} height={layout.height} aria-hidden="true">{layout.links.map(link=><path key={link.id} d={link.d}/>)}{activePoint&&point&&<path className={styles.preview} d={`M ${activePoint.x+164} ${activePoint.y+31} C ${(activePoint.x+164+point.x)/2} ${activePoint.y+31}, ${(activePoint.x+164+point.x)/2} ${point.y+31}, ${point.x} ${point.y+31}`}/>}</svg>
     <SortableContext items={projection.nodes.filter(node=>node.parent_id!==null).map(node=>`node:${node.id}`)} strategy={rectSortingStrategy}>{projection.nodes.map(node=><Positioner key={node.id} node={node} point={layout.points.get(node.id)??{x:0,y:0}} selected={node.id===selected?.id} menuOpen={node.id===menuNodeId} onSelect={()=>{setSelectedId(node.id);setMenuNodeId(current=>current===node.id?undefined:node.id);setEditorOpen(false);}} onAdd={()=>{setSelectedId(node.id);setMenuNodeId(undefined);dialogReturnFocus.current=document.querySelector<HTMLElement>(`[data-life-edit-id="${node.id}"]`);setCreateOpen(true);}} onEdit={()=>{setSelectedId(node.id);setMenuNodeId(undefined);setEditorOpen(true);}} invalid={node.id===activeId||invalidDescendants.has(node.id)} active={Boolean(activeId)}/>)}</SortableContext>
    </div>
   </div>
   {selected&&editorOpen&&<aside className={styles.inspector} aria-label="Life node editor">
    <button type="button" className={styles.closeInspector} aria-label="Close node editor" onClick={()=>{setEditorOpen(false);document.querySelector<HTMLElement>(`[data-life-edit-id="${selected.id}"]`)?.focus();}}><Icon d={iconDismiss} size={14}/></button>
